...
def generate_place_name(min_len=2, max_len=8):
    ...
    for _ in range(1000):
        ...
        while len(name) < target_len:
            ...
            candidates = [mora for mora in MORA_LIST if len(mora) <= remaining]

            if remaining == 1:
                candidates = [mora for mora in ENDING_LIST if len(mora) == 1]
...
def path_crosses_existing_paths(new_path, selected_paths):
    a, b = new_path

    for c, d in selected_paths:
...
def generate_connected_paths(nodes, path_count):
    ...
    while unconnected:
        ...
        for a in connected:
            for b in unconnected:
                ...
                if path_crosses_existing_paths((a, b), selected_paths):
                    continue
        ...
        if not candidates:
            raise RuntimeError(
                "交差なしで全ノードを1つに接続できませんでした。\n"
                "ノード数を減らすか、キャンバスを広げてください。"
            )
    ...
    for pair, weight in zip(all_pairs, all_weights):
        ...
        if path_crosses_existing_paths(pair, selected_paths):
            continue
    ...
    while len(selected_paths) < path_count and remaining_pairs:
        ...
        new_remaining_pairs = []
        new_remaining_weights = []

        for candidate_pair, candidate_weight in zip(remaining_pairs, remaining_weights):
            if not path_crosses_existing_paths(candidate_pair, selected_paths):
                new_remaining_pairs.append(candidate_pair)
                new_remaining_weights.append(candidate_weight)

        remaining_pairs = new_remaining_pairs
        remaining_weights = new_remaining_weights
...
def generate_image():
    try:
        ...
        node_names = {node: generate_place_name(2, 8) for node in nodes}
        city_sizes = {node: str(random.randint(1, 10)) for node in nodes}
